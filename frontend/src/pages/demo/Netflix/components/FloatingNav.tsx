// frontend/src/pages/demo/Netflix/components/FloatingNav.tsx
...
function FloatingNav() {
  return (
    <nav className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-black/85 backdrop-blur-md border border-[#2a2a2a] rounded-full px-2 py-2 shadow-lg shadow-black/40">
      {navItems.map((item) => (
        <button
          key={item.id}
          onClick={() => scrollToId(item.id)}
          className="flex flex-col items-center justify-center w-14 py-1.5 rounded-full text-[#a3a3a3] hover:text-white hover:bg-[#E50914]/20 transition-colors"
        >
          {item.icon}
