  ...
  return (
    ...
    >
      ...
      <form
        ...
        className="w-full max-w-md bg-white rounded-[2rem] shadow-md shadow-[#b5654a]/10 px-6 py-6 flex flex-col gap-4 mb-10"
      >
        <input
          type="text"
          value={nama}
          ...
          className="w-full bg-[#f7efe3] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-[#b5654a]"
        />
        <textarea
          value={pesan}
          ...
          rows={3}
          className="w-full bg-[#f7efe3] rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-[#b5654a] resize-none"
        />
        <button
          type="submit"
          className="self-start bg-[#b5654a] text-white rounded-full px-6 py-2.5 text-sm hover:bg-[#8f4a35] transition-colors"
        >
          Kirim Ucapan
        </button>
      </form>
      ...
      <div className="w-full max-w-md flex flex-col gap-3 max-h-[400px] overflow-y-auto">
        {daftarUcapan.map((u) => (
          <div
            key={u.id}
            className="bg-white rounded-[1.5rem] shadow-sm shadow-[#b5654a]/10 px-5 py-4"
          >
            <p className="font-heading text-sm text-[#b5654a] mb-1">
              {u.nama}
            </p>
            <p className="text-sm text-[#5c4530]">{u.pesan}</p>
          </div>
        ))}
      </div>
    </motion.div>
