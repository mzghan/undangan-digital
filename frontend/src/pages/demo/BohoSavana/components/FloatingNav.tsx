// frontend/src/pages/demo/BohoSavana/components/FloatingNav.tsx
...
function FloatingNav() {
  return (
    <nav className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-white/80 backdrop-blur-md border border-[#e3d5bb] rounded-full px-2 py-2 shadow-lg shadow-[#b5654a]/10">
      {navItems.map((item) => (
        <button
          key={item.id}
          onClick={() => scrollToId(item.id)}
          className="flex flex-col items-center justify-center w-14 py-1.5 rounded-full text-[#9c8b74] hover:text-[#b5654a] hover:bg-[#f0e4d0] transition-colors"
        >
          {item.icon}
