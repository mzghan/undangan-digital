// frontend/src/pages/demo/PuspaBali/components/FloatingNav.tsx
...
function FloatingNav() {
  return (
    <nav className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-white/80 backdrop-blur-md border border-[#e6d9b8] rounded-full px-2 py-2 shadow-lg shadow-[#c9a24b]/10">
      {navItems.map((item) => (
        <button
          key={item.id}
          onClick={() => scrollToId(item.id)}
          className="flex flex-col items-center justify-center w-14 py-1.5 rounded-full text-[#8a7f6b] hover:text-[#c9a24b] hover:bg-[#f7ecd6] transition-colors"
        >
          {item.icon}
