// frontend/src/pages/demo/FloralBlanc/sections/AmplopDigital.tsx
...
function AmplopDigital() {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[#a89a94] max-w-sm text-center mb-12">
      ...
      <div className="w-full max-w-md flex flex-col gap-4">
        {daftarRekening.map((r) => (
          <div
            key={r.id}
            className="bg-white rounded-2xl shadow-md shadow-[#c9a06e]/10 px-5 py-4 flex items-center justify-between"
          >
            <div>
              <p className="font-heading text-lg">{r.bank}</p>
              <p className="text-sm text-[#c9a06e] tracking-wider">
                {r.nomor}
              </p>
              <p className="text-xs text-[#a89a94] mt-1">a.n. {r.atasNama}</p>
            </div>
            <button
              onClick={() => handleCopy(r.nomor, r.id)}
              className="text-xs bg-[#c9a06e] text-white rounded-full px-4 py-2 hover:bg-[#a8794a] transition-colors whitespace-nowrap"
            >
