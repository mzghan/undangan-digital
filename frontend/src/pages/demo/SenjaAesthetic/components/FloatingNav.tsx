// frontend/src/pages/demo/SenjaAesthetic/components/FloatingNav.tsx
...
function FloatingNav() {
  return (
    <nav className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-white/80 backdrop-blur-md border border-[#e9d8cc] rounded-full px-2 py-2 shadow-lg shadow-[#c17b5f]/10">
      {navItems.map((item) => (
        <button
          key={item.id}
          onClick={() => scrollToId(item.id)}
          className="flex flex-col items-center justify-center w-14 py-1.5 rounded-full text-[#8c7b6f] hover:text-[#c17b5f] hover:bg-[#f5e6dc] transition-colors"
        >
          {item.icon}
